...
function environment(extra = {}) {
    ...
    for (const name of [
        ...
        "CAIRN_CONTEXT_PACK_HTTP", "CAIRN_CONTEXT_USAGE", "CAIRN_WORK_EVIDENCE", "CAIRN_CAPABILITY_CONTRACT", "MCP_HTTP_PORT",
    ]) delete env[name];
...
const allGates = await tools(environment({
    CAIRN_TYPED_MEMORY_NODES: "1", CAIRN_ARTIFACT_STORE: "1", CAIRN_CONTEXT_PACKS: "1", CAIRN_CONTEXT_USAGE: "1", CAIRN_WORK_EVIDENCE: "1",
}));
assert.deepEqual(allGates.map(({ name }) => name), MCP_TOOL_NAMES, "whole catalog registration order");
allGates.forEach(complete);

const readOnly = await tools(environment({ CAIRN_MCP_TOOL_PROFILE: "read-only", CAIRN_TYPED_MEMORY_NODES: "1", CAIRN_ARTIFACT_STORE: "1", CAIRN_CONTEXT_PACKS: "1", CAIRN_CONTEXT_USAGE: "1", CAIRN_WORK_EVIDENCE: "1" }));
