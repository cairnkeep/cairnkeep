...
const eventHelp = spawnSync(process.execPath, [cli, "check", "start", "--help"], { encoding: "utf8" });
assert.equal(eventHelp.status, 0, eventHelp.stderr);
assert.match(eventHelp.stdout, /trivial\|standard\|complex/);
const invalidEventHelp = spawnSync(process.execPath, [cli, "check", "start", "finish", "--help"], { encoding: "utf8" });
assert.equal(invalidEventHelp.status, 2);
