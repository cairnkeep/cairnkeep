...
provides:
  ...
  - "A definitive root cause for OCP-04's read-back gap: it is a fundamental tool-calling limitation of qwen3.6-27b-coder for multi-step /remember & /recall flows, NOT fixable by thinking config (triangulated across raw curl, the :8006 proxy, AND direct chat_template_kwargs.enable_thinking=false in opencode's model config): thinking-on gives a reliable /remember write but no /recall read; thinking-off (proxy or direct config, both confirmed reaching vLLM) loses the /remember write entirely. No thinking setting fires both. Genuine fix = a different, more tool-call-reliable local model. Not a defect in recall.md/remember.md/cairn-memory/harness"
...
tech-stack:
  ...
  patterns:
    ...
    - "Thinking-model tool-calling diagnosis: toggling thinking can move WHICH tool call fails rather than fixing it — for qwen3.6-27b-coder, thinking-on fires the write but not the read, thinking-off loses the write; triangulate across raw curl, a proxy, AND direct model-config injection before concluding, and verify the endpoint works with the ACTUAL client (opencode), not just raw curl"
...
coverage:
  ...
  - id: D5
    ...
    verification:
      - kind: manual_procedural
        ref: "05-UAT.md Test 4 + Test 5 — definitive: fundamental qwen3.6-27b-coder tool-calling limitation for multi-step /remember & /recall, NOT fixable by thinking config (triangulated: thinking-on fires write not read; thinking-off via proxy OR direct enable_thinking=false loses the write). Genuine fix = a different, more tool-reliable local model"
        status: fail
    human_judgment: true
    rationale: "OCP-04 read-back is an open, definitively-characterized limitation — deliberately recorded as an open gap for verification to surface, NOT a passing result. The mechanisms (recall.md, cairn-memory server, harness) are proven correct; the limitation is a fundamental tool-calling weakness of qwen3.6-27b-coder for multi-step /remember & /recall flows, proven NOT fixable by thinking config (triangulated: thinking-on fires write not read, thinking-off loses the write). The genuine fix is a different, more tool-call-reliable local model"
  - id: D6
...
- **OCP-04 read-back marked as an open, definitively-characterized limitation**, not a passing result. The recall.md command, the cairn-memory MCP server, and the harness are all proven correct; the limitation is a fundamental tool-calling weakness of qwen3.6-27b-coder for multi-step /remember & /recall flows, proven NOT fixable by thinking config (thinking-on fires the write but not the read; thinking-off loses the write). The genuine fix is a different, more tool-call-reliable local model.
...
- **Fundamental tool-calling limitation of qwen3.6-27b-coder (definitive root cause of OCP-04's gap), NOT fixable by thinking config.** Triangulated across three independent transports — raw `curl`, the `:8006` thinking-strip proxy, AND direct `chat_template_kwargs.enable_thinking=false` injection in `opencode`'s own model config (which DOES reach vLLM, proven by the behavior change): **Thinking ON** (`:8001` default) — `/remember` fires a real `memory_write` reliably, but `/recall` does NOT fire `memory_search`/`_read` (the model narrates it). **Thinking OFF** (via the proxy OR via direct `enable_thinking=false` config) — `/remember`'s `memory_write` STOPS firing (~4 trials across proxy + direct config); the read half is never even reached. So there is NO thinking setting where both write and read fire for these multi-step slash-command flows: thinking-on buys the write and loses the read; thinking-off loses the write. **Fix direction (documented, not implemented):** the genuine remaining fix is a DIFFERENT, more tool-call-reliable local model — not this coder with thinking toggled; thinking config is a proven dead-end. (Secondary, now moot: `opencode`'s Node/undici client also hangs on the `:8006` uvicorn/FastAPI proxy across framing/injection/keep-alive variants while raw `curl` works — but that no longer matters given thinking-off breaks writes regardless of transport.) `.ai/.env` was reverted to the stable `:8001` coder endpoint matching the committed `--full` evidence.
...
None for this plan. Note for whoever revisits OCP-04's read-back: a DIFFERENT, more tool-call-reliable local model is the prerequisite to closing that gap — thinking config on this coder is a proven dead-end (see fix direction above). The operator must provision such a model; it is outside the executor's scope.
...
- **Open, documented gap for verification/milestone-audit to surface:** OCP-04's recall live read-back, root-caused to a fundamental qwen3.6-27b-coder tool-calling limitation for multi-step /remember & /recall flows that thinking config cannot fix (not a code defect; genuine fix is a different, more tool-reliable local model), plus the never-run interactive TUI session (D-01 fallback-gap). Both are recorded explicitly in 05-UAT.md, not silently dropped.
