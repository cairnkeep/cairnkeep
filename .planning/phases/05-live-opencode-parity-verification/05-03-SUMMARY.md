...
provides:
  ...
  - "A root cause for OCP-04's read-back gap: qwen3.6-27b-coder is a thinking model whose reasoning leaks as narrated pseudo-tool-call text (thinking-on/:8001 breaks /recall read; a thinking-strip proxy/:8006 fires an isolated read cleanly but breaks the multi-step /remember write) — a model-reliability limitation, not a defect in recall.md/remember.md/cairn-memory/harness"
...
tech-stack:
  ...
  patterns:
    ...
    - "Thinking-model tool-calling diagnosis: an isolated single-step tool call may fire on a thinking-stripped endpoint while the same model's multi-step slash-command flow does not — endpoint-level thinking toggle is not sufficient for reliable multi-step agentic tool use"
...
coverage:
  ...
  - id: D5
    ...
    verification:
      - kind: manual_procedural
        ref: "05-UAT.md Test 4 + Test 5 — thinking-model root cause: reasoning leaks as narrated pseudo-tool-call text; write half fires on :8001, read half does not; thinking-strip proxy fires isolated read but breaks multi-step write"
        status: fail
    human_judgment: true
...
- **Thinking-model tool-calling unreliability (root cause of OCP-04's gap).** `qwen3.6-27b-coder` is a reasoning model; on the thinking-on endpoint (`:8001`) its reasoning leaks out as narrated pseudo-tool-call text instead of genuine tool-call events. `/remember` (write) still fires a real `memory_write` on `:8001`; `/recall` (read) does not fire `memory_search`/`_read`. A thinking-strip proxy (`enable_thinking=false`, `:8006`) makes an isolated read call fire cleanly (`finish_reason=tool_calls`, `tool_calls=[memory_search]`) in a raw `/v1/chat/completions` probe, but driving the full multi-step `/remember` slash-command flow through opencode against `:8006` made the write call stop firing (0/3). Neither endpoint yields a reliable full round-trip through opencode's multi-step slash commands. **Fix direction (documented, not implemented):** a tool-call-reliable local config — provider-level thinking-disable that preserves multi-step tool-calling, or a model with robust agentic tool use. `.ai/.env` was reverted to the stable `:8001` coder endpoint matching the committed `--full` evidence.
