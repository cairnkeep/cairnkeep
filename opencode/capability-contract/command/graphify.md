...
This contract-only block takes precedence over every legacy instruction below,
including the instruction to begin Step 0. Run it before any banner, output,
repository discovery, legacy graph config read, filesystem access, or tool call.

1. Use Bash to run exactly:

   ```bash
   capability_guard=$(cairn capabilities guard graph --harness opencode --source operating-command --transport harness-command)
   ```

   Do not pass `$ARGUMENTS`, paths, query terms, results, errors, or metadata.
2. Parse the strict JSON result. If `disabled` is `true`, present that exact JSON
   object as the sole stable disabled result and **STOP** before the banner and
   before the legacy `.planning/config.json` gate.
3. Immediately before entering the unchanged legacy graph owner body below,
   use Bash to run exactly:

   ```bash
   capability_start=$(cairn capabilities start graph --harness opencode --source operating-command --transport harness-command)
   ```

   If this second state check returns `disabled: true`, present that exact JSON
   object and **STOP**. If it contains `invocation_id`, retain the entire strict
   JSON object unchanged as `capability_handle`; otherwise the invocation is
...
4. After the legacy owner settles, classify only its terminal outcome as
   ...
   logging cannot change the owner's result. Never retry finish or pass
   arguments, paths, query terms, results, raw errors, or arbitrary metadata.
   Every STOP, returned error, and timeout after start follows this final-only
   rule.

Guard resolution and overlay setup occur before measurement. Reset/inherit state
continues to use the existing explicit `graphify.enabled === true` compatibility
gate in the unchanged owner body.
