...
runs before required-reading expansion, output, repository discovery, config
reads, filesystem access, tools, agents, or other owner work.

1. An inherited `capability_handle` is accepted only when it is the untouched
   strict JSON object returned by the immediately delegating installed wiki
   command's `cairn capabilities start` call. It must not originate in
   `$ARGUMENTS`, user text, a path, a query, or arbitrary metadata. A valid
   inherited handle already represents the command's pre-delegation state check;
   do not guard or start again.
2. Without that trusted bounded handle, run exactly:

   ```bash
   capability_guard=$(cairn capabilities guard wiki --harness opencode --source operating-workflow --transport harness-command)
   ```

   Pass no arguments, paths, queries, results, errors, or metadata. If
   `disabled` is `true`, present that exact JSON object as the sole stable result
   and **STOP** before entering the legacy workflow.
3. For an enabled direct invocation, immediately before the unchanged owner body
   run exactly:

   ```bash
   capability_start=$(cairn capabilities start wiki --harness opencode --source operating-workflow --transport harness-command)
   ```
...
Guard resolution is outside measurement. The legacy workflow retains all owner
behavior, tools, artifacts, errors, and presentation.
