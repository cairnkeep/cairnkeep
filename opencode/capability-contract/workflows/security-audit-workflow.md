...
This contract-only block takes precedence over the legacy workflow below and
runs before required-reading expansion, output, repository discovery, provider
or config resolution, filesystem access, tools, agents, or owner work.

1. An inherited `capability_handle` is accepted only when it is the untouched
   strict JSON object returned by the immediately delegating installed security
   command's `cairn capabilities start` call. It must not originate in
   `$ARGUMENTS`, user text, a path, a threat model, a finding, or arbitrary
   metadata. A valid inherited handle already represents the command's
   pre-delegation state check; do not guard or start again.
2. Without that trusted bounded handle, run exactly:

   ```bash
   capability_guard=$(cairn capabilities guard security.audit --harness opencode --source operating-workflow --transport harness-command)
   ```

   Pass no arguments, paths, threat models, findings, results, errors, or
   metadata. If `disabled` is `true`, present that exact JSON object as the sole
   stable result and **STOP** before entering the legacy workflow.
3. For an enabled direct invocation, immediately before the unchanged owner body
   run exactly:

   ```bash
   capability_start=$(cairn capabilities start security.audit --harness opencode --source operating-workflow --transport harness-command)
   ```

   If it returns `disabled: true`, present that exact object and **STOP**. Retain
   an `invocation_id` result unchanged as `capability_handle`; an unmeasured
   result has no handle.
4. After the owner settles as `success`, `error`, or `timeout`, and before final
   presentation, invoke `cairn capabilities finish --handle "$capability_handle" --outcome <outcome>`
   exactly once when a handle exists. Suppress failure and never retry. Pass no
   owner data or raw error. This workflow is the sole finalization owner for both
   direct and command-delegated invocations.

Guard resolution is outside measurement. The legacy workflow retains all
policy, agent, artifact, provider-neutral, error, and presentation ownership.
