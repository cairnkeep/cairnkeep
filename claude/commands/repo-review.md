---
description: Review code for bugs, security issues, and maintainability concerns (workflow code-review; distinct name to avoid shadowing the built-in /code-review)
...
Spawn the `code-reviewer` subagent via the Task tool with `subagent_type: "code-reviewer"`.
...
Prompt structure:
```text
Read agent instructions for code review.
...
▶ /repo-review --post-to-mr to comment on MR
```
