...
This block is the contract-only overlay and takes precedence over every legacy
instruction below, including the instruction to begin Step 0. Run it before any
banner, output, repository discovery, legacy graph config read, filesystem
access, or other tool call.

1. Use Bash to run exactly:

   ```bash
   capability_guard=$(cairn capabilities guard graph --harness claude-code --source operating-command --transport harness-command)
   ```
...
3. Immediately before entering the unchanged legacy graph owner body below,
   ...
   ```bash
   capability_start=$(cairn capabilities start graph --harness claude-code --source operating-command --transport harness-command)
   ```
...
4. After the legacy owner settles, classify only its terminal outcome as
   ...
   logging cannot change the owner's result. Never pass arguments, paths, query
   terms, results, raw errors, or arbitrary metadata to finish. Every STOP,
   returned error, and timeout after start follows this same final-only rule.
