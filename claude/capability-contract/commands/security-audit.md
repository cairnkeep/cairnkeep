...
This block is the contract-only overlay. It takes precedence over the unchanged
legacy command below and must run before provider resolution, output, repository
discovery, config reads, filesystem access, tool calls, or agent delegation.

1. Use Bash to run exactly:

   ```bash
   capability_guard=$(cairn capabilities guard security.audit --harness claude-code --source operating-command --transport harness-command)
   ```

   Do not pass `$ARGUMENTS`, paths, target details, findings, results, errors, or
   other metadata.
...
3. Immediately before entering the unchanged legacy security owner body below,
   use Bash to run exactly:

   ```bash
   capability_start=$(cairn capabilities start security.audit --harness claude-code --source operating-command --transport harness-command)
   ```
...
4. After the legacy owner settles, classify only its terminal outcome as
   ...
   findings, results, raw errors, or arbitrary metadata to finish. Every STOP,
   returned error, and timeout after start follows this same final-only rule.

Guard resolution and overlay setup occur before measurement. The legacy
security agents, workflow, artifacts, and provider-neutral behavior remain
owned by the unchanged command below.
