...
grep -qF 'review.security' "$tmp/playbook-record-help" || fail "playbook record help missing actions"
if "$cairn" playbook unknown --help >/dev/null 2>&1; then
  fail "unknown playbook command help should fail"
fi
